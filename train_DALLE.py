...
def main():
    ...
    def encode_batch(imgs):
        z_q, indices = vq.encode(imgs, params=params)

        if indices.ndim != 2 or indices.shape[1] != 256:
            raise ValueError(f"Expected indices shape (B,256), but got {tuple(indices.shape)}")
        return indices
